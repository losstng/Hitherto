...
The sidebar also links to an **Analytics** page which provides a simple Jupyter-style notebook backed by the FastAPI API. The notebook view includes a side panel to toggle between a list of saved notebooks and the variables currently defined in the running session.  Saved notebooks can now be opened, renamed or deleted from this panel.
