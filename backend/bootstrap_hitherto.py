""" Hitherto — ALL schemas in one file (PostgreSQL, SQLAlchemy 2.x)
Covers:
A) core: asset, regime_state, playbook, policy_proposal, decision, portfolio_snapshot
B) signals: module, snapshot (PARTITIONED by month), fusion_trace
C) risk: limits, checks, verdict
D) models: feature_def, feature_store (PARTITIONED), hmm_spec, hmm_params, bayes_model, training_run, inference_trace
E) audit: event (PARTITIONED), change_log
F) ops: review_queue, review_action, run_status

Conventions:
- BIGINT surrogate keys via Identity.
- timestamptz everywhere; session_date/session_id where useful.
- JSONB for payloads.
- Partial unique: one active playbook.
- Hot-path indexes.
- Monthly partitions for: signals.snapshot, models.feature_store, audit.event.
...
from this_file import bootstrap_hitherto
...
core_regime_state = Table(
    ...
    Column('source', sa.Text, nullable=False), # "HMM" | "Human" | ...
...
# Optional rolling partial (static snapshot of "now()"); recreate on a cadence if desired.
Index(
...
signals_module = Table(
    ...
    Column('family', sa.Text, nullable=False), # sentiment|fundamental|technical|macro|alt|...
...
CREATE TABLE IF NOT EXISTS signals.snapshot (
    ...
    asset_id BIGINT REFERENCES core.asset(id) ON DELETE SET NULL, -- null for macro
...
risk_limits = Table(
    ...
    Column('id', sa.BigInteger, sa.Identity(always=False), primary_key=True),
    Column('created_at', TIMESTAMPTZ, nullable=False, server_default=text('now()')),
    Column('scope', sa.Text, nullable=False),  # "portfolio|sector|asset|exposure|VaR"
...
models_feature_def = Table(
    ...
    Column('created_at', TIMESTAMPTZ, nullable=False, server_default=text('now()')),
    Column('name', sa.Text, nullable=False, unique=True),
    Column('spec', JSONB, nullable=False, server_default=text("'{}'::jsonb")),  # formula, window, universe
    Column('owner', sa.Text),
    Column('is_active', sa.Boolean, nullable=False, server_default=text('true')),
    schema='models'
)
...
models_hmm_spec = Table(
    ...
    Column('obs_family', sa.Text, nullable=False),  # "Gaussian|GMM"
    Column('regime_labels', JSONB, nullable=False, server_default=text("'{}'::jsonb")),  # state->regime
...
models_training_run = Table(
    ...
    Column('id', sa.BigInteger, sa.Identity(always=False), primary_key=True),
    Column('created_at', TIMESTAMPTZ, nullable=False, server_default=text('now()')),
    Column('model_kind', sa.Text, nullable=False),  # "HMM|BMA|CPD|..."
    Column('spec_id', sa.BigInteger),  # context-dependent FK (nullable)
...
models_hmm_params = Table(
    ...
    Column('pi', pg.ARRAY(DOUBLE), nullable=False),  # double precision[]
    Column('a', pg.ARRAY(DOUBLE, dimensions=2), nullable=False),  # double precision[][]
    Column('emission', JSONB, nullable=False, server_default=text("'{}'::jsonb")),  # means, covariances
...
models_bayes_model = Table(
    ...
    Column('name', sa.Text, nullable=False),
    Column('version', sa.Integer, nullable=False),
    Column('prior', JSONB, nullable=False, server_default=text("'{}'::jsonb")),  # module -> prior weight
...
models_inference_trace = Table(
    ...
    Column('id', sa.BigInteger, sa.Identity(always=False), primary_key=True),
    Column('created_at', TIMESTAMPTZ, nullable=False, server_default=text('now()')),
    Column('asof', TIMESTAMPTZ, nullable=False),  # regime inference
    Column('hmm_params_id', sa.BigInteger, ForeignKey('models.hmm_params.id', ondelete='SET NULL')),
    Column('state_post', pg.ARRAY(DOUBLE)),  # gamma_t
...
CREATE TABLE IF NOT EXISTS audit.event (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    asof timestamptz NOT NULL,
    emitter TEXT NOT NULL, -- "Overseer|Risk|Human|Daemon"
...
def _ensure_monthly_partitions(
    conn, parent: str, from_yyyymm: str, to_yyyymm: str
) -> None:
    """Create monthly RANGE partitions [from_yyyymm, to_yyyymm] for a table
    partitioned by (asof).

    parent: 'schema.table' (e.g., 'signals.snapshot')
    """
...
def bootstrap_hitherto(
    ...
    """Create all schemas, enums, tables, partition parents + indexes,
    and (optionally) monthly partitions.

    partitions keys (optional):
    - 'signals_snapshot': ('YYYYMM_from', 'YYYYMM_to')
    - 'models_feature_store': ('YYYYMM_from', 'YYYYMM_to')
    - 'audit_event': ('YYYYMM_from', 'YYYYMM_to')
    """
    ...
    with engine.begin() as conn:
        ...
        # Partition-parent indexes (inherit to partitions)
        ...
        # Create monthly partitions if requested
        if partitions:
...
if __name__ == "__main__":
    # Example (disabled):
    # DATABASE_URL = "postgresql+psycopg://user:pass@localhost:5432/hitherto"
    # bootstrap_hitherto(DATABASE_URL, partitions={
    #     "signals_snapshot": ("202401", "202612"),
    #     "models_feature_store": ("202401", "202612"),
    #     "audit_event": ("202401", "202612"),
    # })
